...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        highestValue = self.computeValueFromQValues(state)
        highestActions = []
        for action in self.getLegalActions(state):
            if self.getQValue(state, action) == highestValue:
                highestActions.append(action)
        if len(highestActions) == 0:
            return None
    ...
    def getAction(self, state):
        ...
        # Have an epsilon probability of picking a random action
