...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    * Please read learningAgents.py before reading this.*
    ...
    def __init__(self, mdp, discount=0.9, iterations=100):
        ...
        for _ in range(iterations):
            for state in mdp.getStates():
                copy[state] = self.mdp.getReward(
                    state, None, None
                ) + self.computeHighestFromValuesExcludingZero(state)
            self.values = copy.copy()

    def isExitState(self, state):
        return "exit" in self.mdp.getPossibleActions(state)
    ...
    def computeHighestFromValuesExcludingZero(self, state):
        ...
        for action in self.mdp.getPossibleActions(state):
            ...
            for nextState, probability in self.mdp.getTransitionStatesAndProbs(
                ...
                reward += self.getValue(nextState) * probability * self.discount
            if (
                highestReward < 0
                and highestReward > float("-inf")
                and reward == 0
                and state not in self.updated
            ):
                continue
